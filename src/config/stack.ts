 ...
 *
 * Son los LOGOS ORIGINALES, a todo color, de la coleccion `logos` de
 * Iconify. Antes eran glifos monocromos que solo tomaban color al pasar
 * el cursor.
 *
 * TRES MARCAS SE QUEDAN EN MONOCROMO, y no por descuido:
 *
 *   - Railway  su logo original es un negro casi puro (#100f13) y sobre
 *              el fondo marino del tema oscuro desaparece.
 *   - Astro    su marca es un cohete oscuro con un degradado encima: en
 *              oscuro se hunde el cuerpo y queda flotando el degradado.
 *   - Spring Security  no tiene logo propio en la coleccion, y repetir la
 *              hoja de Spring Boot pondria dos iconos identicos en la
 *              rejilla.
 *
 * Esas tres usan el glifo de `simple-icons`, que se pinta con
 * `currentColor` y por tanto se adapta al tema. Vercel usa su logo
 * original y funciona igual de bien: su marca no lleva color fijo.
 *
 * El color de marca de abajo sigue haciendo falta para el borde de la
 * celda al pasar el cursor, y para tintar esas tres monocromas.
 *
